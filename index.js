...
const { travar } = require('./src/travar')
const { help } = require('./src/help')
const { trava1 } = require('./iwouky/trava1')
const { trava2 } = require('./iwouky/trava2')
const { trava3 } = require('./iwouky/trava3')
const { trava4 } = require('./iwouky/trava4')
const { trava5 } = require('./iwouky/trava5')
const { trava6 } = require('./iwouky/trava6')
const { trava7 } = require('./iwouky/trava7')
const { trava8 } = require('./iwouky/trava8')
const { trava9 } = require('./iwouky/trava9')
const { trava10 } = require('./iwouky/trava10')
const { trava11 } = require('./iwouky/trava11')
const { trava12 } = require('./iwouky/trava12')
const {help1} = require('./src/help1')
const { modapk } = require('./src/modapk')
const { pack } = require('./src/pack')
const { outros } = require('./src/outros')
const { playlist } = require('./src/playlist')
const { infocovid } = require('./src/infocovid')
const { videolist } = require('./src/videolist')
const { pais } = require('./src/pais')
const { regras } = require('./src/regras')
const { vipmenu } = require('./src/vipmenu')
const { destrava } = require('./src/destrava')
const { wppimune } = require('./src/wppimune')
const { appsmods } = require('./src/appsmods')
const { geradores } = require('./src/geradores')
const { appsmods2 } = require('./src/appsmods2')
const { contatos } = require('./src/contatos')
const { sites } = require('./src/sites')
const { contasdisney } = require('./src/contasdisney')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:iWouKy\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558282042647:+55 8282042647\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = "."
blocked = []
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} 𝙵𝙾𝙸 𝚃𝙰𝚁𝙳𝙴 𝙲𝚁𝙾𝙲𝙾𝙳𝙸𝙻𝙾 𝙳𝙾 𝙺𝚁𝙻😡`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ Espere Caso nao funcione tente novamente❬❗❭',
					success: '️❬ ✔ ❭ Sucesso 👍',
				...
				only: {
					...
					premium: '[❗] CALA BOCA MEMBRO COMUM,VC N É PREMIUM😈*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD DAVI BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Davi? Este é um recurso especial para o Davi ❌',
					ownerB: '[❗] Davi? Este é um recurso especial para o Davi ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de DAVI. Registre-se para fazer amizade com o bot DAVI por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar iWouKy|17\n\n──「 LEVI-BOT 」──`,
			...
			const ownerNumber = ["558282042647@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558282042647@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558282042647@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558282042647@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558282042647@s.whatsapp.net","553299629633@s.whatsapp.net","558193627089@s.whatsapp.net"]
			const cassino = ['ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 1 ─═─ 2 ─═─ 3*\n*║*\n*║*\n*╠* Não foi dessa vez mas\n*║* continue tentando.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 2 ─═─ 3 ─═─ 1*\n*║*\n*║*\n*╠* Não foi dessa vez mas\n*║* continue tentando.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 3 ─═─ 2 ─═─ 1*\n*║*\n*║*\n*╠* Não foi dessa vez mas\n*║* continue tentando.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 1 ─═─ 3 ─═─ 2*\n*║*\n*║*\n*╠* Não foi dessa vez mas\n*║* continue tentando.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 2 ─═─ 1 ─═─ 3*\n*║*\n*║*\n*╠* Não foi dessa vez mas\n*║* continue tentando.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 3 ─═─ 1 ─═─ 2*\n*║*\n*║*\n*╠* Não foi dessa vez mas\n*║* continue tentando.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 1 ─═─ 1 ─═─ 1*\n*║*\n*║*\n*╠* PARABÉNS !!!\n*╠* VOCÊ GANHOU NO CASSINO.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 2 ─═─ 2 ─═─ 2*\n*║*\n*║*\n*╠* PARABÉNS !!!\n*╠* VOCÊ GANHOU NO CASSINO.\n*║*\n*╚═─ CASSINO ─══*','ㅤ\n*╔═─ CASSINO ─══*\n*║*\n*║*\n*╠* ROLETA DOS TRÊS\n*╠* NÚMEROS\n*║*\n*╠═─ 3 ─═─ 3 ─═─ 3*\n*║*\n*║*\n*╠* PARABÉNS !!!\n*╠* VOCÊ GANHOU NO CASSINO.\n*║*\n*╚═─ CASSINO ─══*']
			const resposta = ['Sim','Não','Talvez','Talvez sim','Talvez não','Tente perguntar novamente','Nem pensar','Tente perguntar novamente','Certamente','Jamais','Nunca']
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes(";-;")){
			client.updatePresence(from, Presence.composing)
			reply("isso e coisa de doido mano")
	}
	
	if (messagesC.includes("😐")){
			client.updatePresence(from, Presence.composing)
			reply("Esse emoji qm usa e gay mano")
	}
	
	
	if (messagesC.includes("novoaqui")){
			client.updatePresence(from, Presence.composing)
			reply("seja bem vindo ao hospicio")
	}
	
	if (messagesC.includes("kraii")){
			client.updatePresence(from, Presence.composing)
			reply("olha a boca man")
	}
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("acordagp")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/acordagp.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("branquinha")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/branquinha.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("ysnfab")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/ysnfab.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("powfu")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/powfu.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("eredeze")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/eredeze.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("rama")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rama.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("geme2")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/geme2.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("despacito")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/despacito.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("360")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/360.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("24kgoldn")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/24kgoldn.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	if (messagesC.includes("yasmin")){
			client.updatePresence(from, Presence.composing)
			reply("melhor cunha🙆‍♂️❤")
	}
	...
	if (messagesC.includes("fcc")){
			client.updatePresence(from, Presence.composing)
			reply("ainda existe isso? KKKKKKK")
	}
	
	if (messagesC.includes("jarlys")){
			client.updatePresence(from, Presence.composing)
			reply("odeio ele🤬")
	}
	...
	if (messagesC.includes("davi")){
			client.updatePresence(from, Presence.composing)
			reply("ctt do meu mestre ai wa.me/558282042647 chama nao")
	}
	
	if (messagesC.includes("Jo💕")){
			client.updatePresence(from, Presence.composing)
			reply("amo ela na moral😏")
	}
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("tmnc")){
			client.updatePresence(from, Presence.composing)
			reply("Toma no cu e caruru 7 pica no teu cu")
	}
	
		if (messagesC.includes("filhodamãe")){
			client.updatePresence(from, Presence.composing)
			reply("logico seu burro")
	}
	
		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("bora juntos fdp?")
	}
	
		if (messagesC.includes("cadeobot?")){
			client.updatePresence(from, Presence.composing)
			reply("olha eu aqui kraii")
	}
	
		if (messagesC.includes("but")){
			client.updatePresence(from, Presence.composing)
			reply("e bot seu burro")
	}
	
		if (messagesC.includes("bot")){
			client.updatePresence(from, Presence.composing)
			reply("Olha eu aqui man, no que posso ajudar?")
	}
			...
			if (!isGroup && isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'do fdp', color(sender.split('@')[0]), 'args :', color(args.length))
			if (!isGroup && !isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mRECV\x1b[1;37m]', time, color('Message'), 'do fdp', color(sender.split('@')[0]), 'args :', color(args.length))
			if (isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'do fdp', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			if (!isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mRECV\x1b[1;37m]', time, color('Message'), 'do fdp', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			switch(command) {
			case 'help':
					case 'menu':
                    me=client.user 
                    uptime=process.uptime 
                       client.sendMessage(from, help(pushname,prefix), text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*DAVI DOMINA VADIA  CRIADOR-DAVI*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
                    break
				case 'help1':
				case 'menu1':
				...
				case 'walpaperhd':
                    ...
                    case 'efeito': 
			case 'efeitin': 
		         if (args.length < 1) return reply('*Digite Com O Texto Que Deseja ✓*')
				lucasssdomina = body.slice(8)
				reply('*_⏳ Preparando Imagem... ⏳_*')
				client.updatePresence(from, Presence.composing) 
				lucasssdomina1 = await fetchJson(`https://luc4riio.herokuapp.com/api/text3d?text=${lucasssdomina}`)
				lucasssdomina2 = await getBuffer(lucasssdomina1.results)
				client.sendMessage(from, lucasssdomina2, image, {quoted: mek, caption: '*_Eaiii, ${pushname}_*\n\n*_Resultado Da Personalização:_*\n*_${args[0]}_*'})
				...
				case 'ramaljadian':
                   ...
                   case 'modapk':
                    ...
                    break
                       tujuh = fs.readFileSync('./assets/edit10.mp4');
                       client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
                     break
                       tujuh = fs.readFileSync('./assets/edit16.mp4');
                       client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
                     break
                     case 'shit3':
                     ...
                     break
                   case 'teto':
                     tujuh = fs.readFileSync('./assets/teto.mp4');
                    client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
                   break
                   case 'teto2':
                   ...
                   case 'destrava':
                    ...
                    case 'wppimune':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, wppimune(prefix), text, { quoted: mek })
                    break
                    case 'appsmods':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, appsmods(prefix), text, { quoted: mek })
                    break
                    case 'geradores':  
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, geradores(prefix), text, { quoted: mek })
                    break
                    case 'appsmods2':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, appsmods2(prefix), text, { quoted: mek })
                    break
                    case 'sites':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, sites(prefix), text, { quoted: mek })
                    break
                    case 'contatos':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, contatos(prefix), text, { quoted: mek })
                    break
                    case 'contasdisney':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, contasdisney(prefix), text, { quoted: mek })
                    break
                    case 'destrava2':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, destrava2(prefix), text, { quoted: mek })
                    break
                   case 'pack':
                    if (!isPremium) return reply(mess.only.premium)
                  ...
                  case 'sttc':
			    	...
			    	client.sendMessage(from, attp2, sticker, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*AQUI ESTÁ SUA FIGURA*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
			     	break
                   case 'gcpf':
                    if (!isPremium) return reply(mess.only.premium)
				...
				case 'ytmp4':
					...
					reply('Espera um pouco krai ser dar erro tenta dnv')
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Apenas foto, que não seja do pau pfvr 🤝')
					}
					break
		case 'iri':
...
//@iWouKyYT					
    ...
    break
				...
				case 'textblue':
					...
					break
					case 'playlist':
					client.sendMessage(from, playlist(prefix) , text, { quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*DAVI DOMINA NE VIDA*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
					break
					case 'infocovid':
					client.sendMessage(from, infocovid(prefix) , text, { quoted: mek })
					break
					case 'videolist':
					client.sendMessage(from, videolist(prefix) , text, { quoted: mek })
					break
					case 'regras':
					client.sendMessage(from, regras(prefix) , text, { quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*AS REGRAS AI KRAI*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
					break
					case 'pais':
					client.sendMessage(from, pais(prefix) , text, { quoted: mek })
					break
					case 'menu2':
	       ...
	       case 'anonimous': 
		         if (args.length < 1) return reply('*Digite Com O Texto Que Deseja ✓*')
				lucasssdomina = body.slice(11)
				reply('*_⏳ Preparando Imagem ⏳_*')
				lucasssdomina1 = await fetchJson(`http://luc4riio.herokuapp.com/api/textoanomymous?text=${lucasssdomina}`)
				lucasssdomina2 = await getBuffer(lucasssdomina1.Resultado)
				client.sendMessage(from, lucasssdomina2, image, {quoted: mek, caption: '*_Resultado Da Busca:_*\n*_${args[0]}_*'})
				await limitAdd(sender)
				break
				case 'jogo':
		...
		case 'bpfont':
			...
			break 
                ...
                case 'mgstravar':
                    ...
                    client.sendMessage('553388728757@s.whatsapp.net', options, text, {quoted: mek})
...
break
case 'daumasugada':
tujuh = fs.readFileSync('./bi/daumasugada.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ponta':
...
break
case 'mood':
tujuh = fs.readFileSync('./bi/mood.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'moskau':
tujuh = fs.readFileSync('./bi/moskaump3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'payplay':
tujuh = fs.readFileSync('./bi/payplay.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'polishcow':
tujuh = fs.readFileSync('./bi/polishcow.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'rasputin':
tujuh = fs.readFileSync('./bi/rasputin.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'onich':
...
break
case 'shitpostgirl':
tujuh = fs.readFileSync('./bi/shitpostgirl.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'tuebaiano':
tujuh = fs.readFileSync('./bi/tuebaiano.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'urss':
tujuh = fs.readFileSync('./bi/urss.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'wthisdown':
tujuh = fs.readFileSync('./bi/wthisdown.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'yamete':
tujuh = fs.readFileSync('./bi/yamete.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'yougirlistenme':
tujuh = fs.readFileSync('./bi/yougirlistenme.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'blindligthpneu':
tujuh = fs.readFileSync('./bi/blindligthpneu.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'buttercnbam':
tujuh = fs.readFileSync('./bi/buttercnbam.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'cunouno':
tujuh = fs.readFileSync('./bi/cunouno.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'dstartnowpneu':
tujuh = fs.readFileSync('./bi/dstartnowpneu.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'autistar':
tujuh = fs.readFileSync('./bi/autistar.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ftuberculose':
tujuh = fs.readFileSync('./bi/ftuberculose.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'paypal':
tujuh = fs.readFileSync('./bi/paypal.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'goticrabuda':
tujuh = fs.readFileSync('./bi/goticrabuda.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'hentaigirl':
tujuh = fs.readFileSync('./bi/hentaigirl.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'hentaigirl2':
tujuh = fs.readFileSync('./bi/hentaigirl2.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ilovedarla':
tujuh = fs.readFileSync('./bi/ilovedarla.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'kalinka':
tujuh = fs.readFileSync('./bi/kalinka.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'katyusha':
tujuh = fs.readFileSync('./bi/katyusha.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'amgdamuie':
tujuh = fs.readFileSync('./bi/amgdamuie.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'antikpop':
tujuh = fs.readFileSync('./bi/antikpop.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'audio':
tujuh = fs.readFileSync('./bi/audio.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ola':
...
if(!isGroup) return reply(mess.only.group)
d = []
teks = '*OS MAIS BOTS DO GRUPOS*💩\n'
for(i = 0; i < 5; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `️‍️‍💩❧ @${groupMembers[r].jid.split('@')[0]}\n`
...
if(!isGroup) return reply(mess.only.group)
d = []
teks = '*OS MAIS PRO PLAYERS DO GP*🔥\n'
for(i = 0; i < 5; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `️‍️‍🔥❧ @${groupMembers[r].jid.split('@')[0]}\n`
d.push(groupMembers[r].jid)
...
case 'tmctt':
                ...
                case 'travar':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, travar(prefix, sender), text, {quoted: mek})
                break
                case 'trava1':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava1(prefix, sender), text, {quoted: mek})
                break
                case 'trava2':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava2(prefix, sender), text, {quoted: mek})
                break
                case 'trava3':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava3(prefix, sender), text, {quoted: mek})
                break
                case 'trava4':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava4(prefix, sender), text, {quoted: mek})
                break
                case 'trava5':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava5(prefix, sender), text, {quoted: mek})
                break
                case 'trava6':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava6(prefix, sender), text, {quoted: mek})
                break
                case 'trava7':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava7(prefix, sender), text, {quoted: mek})
                break
                case 'trava8':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava8(prefix, sender), text, {quoted: mek})
                break
                case 'trava9':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava9(prefix, sender), text, {quoted: mek})
                break
                case 'trava10':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava10(prefix, sender), text, {quoted: mek})
                break
                case 'trava11':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava11(prefix, sender), text, {quoted: mek})
                break
                case 'trava12':
                         if (!isPremium) return reply(mess.only.premium)
		if	(isPremium)client.sendMessage(from, trava12(prefix, sender), text, {quoted: mek})
                break
                case 'porno5':
  reply('Espera um pouco krai ser dar erro tenta dnv')
if (!isPremium) return reply(mess.only.premium)
if (isPremium)tujuh = fs.readFileSync('./assets/porno5.mp4');
client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
case 'porno1':
reply('Espera um pouco krai ser dar erro tenta dnv')
if (!isPremium) return reply(mess.only.premium)
if (isPremium)tujuh = fs.readFileSync('./assets/porno1.mp4');
client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
case 'porno2':
reply('Espera um pouco krai ser dar erro tenta dnv')
if (!isPremium) return reply(mess.only.premium)
if (isPremium)tujuh = fs.readFileSync('./assets/porno2.mp4');
client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
case 'porno3':
reply('Espera um pouco krai ser dar erro tenta dnv')
if (!isPremium) return reply(mess.only.premium)
if (isPremium)tujuh = fs.readFileSync('./assets/porno3.mp4');
client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
case 'porno4':
reply('Espera um pouco krai ser dar erro tenta dnv')
if (!isPremium) return reply(mess.only.premium)
if (isPremium)tujuh = fs.readFileSync('./assets/porno4.mp4');
client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
                case 'trabalho': 
				case 'caderno':
                ...
                case 'nazista':
					...
					const ab =['500%', '15%', '20%', '32%', '34%', '39%', '41%', '49%', '50%', '53%', '55%', '61%', '67%', '69%', '74%', '75%', '80%', '88%', '90%', '93%', '99%', '100%']
					const be = ab[Math.floor(Math.random() * ab.length)] 
					buffer = await getBuffer(`https://i.ibb.co/f4Dj3K5/download.jpg`)
					client.sendMessage(from, buffer, image, { quoted: mek, caption:`*‍ Porcetagem Nazista*\n_${pushname} você é `+be +` Nazista😳🔥._`}) 
					break
				case 'hinata':
              ...
              reply(hasil)
				...
				client.sendMessage(from, buffer, image, {caption: 'aqui está:)', quoted: mek})
                 ...
                 case 'ddd':
                    ...
                    break
                 case 'shitpost2':
					data = fs.readFileSync('./outros/shitpost2.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 buffer = await getBuffer(randKey.result)
				client.sendMessage(from, buffer, image, {caption: 'aqui está:)', quoted: mek})
                 ...
                 if (!isPremium) return reply(mess.only.premium)
                 reply('Espera um pouco krai ser dar erro tenta dnv')
			     data = fs.readFileSync('./outros/hentai2.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 buffer = await getBuffer(randKey.result)
				client.sendMessage(from, buffer, image, {caption: 'aqui está 😈', quoted: mek})
                 break
                 case 'shitpost3':
					data = fs.readFileSync('./outros/shitpost3.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 buffer = await getBuffer(randKey.result)
...
break
		        ...
		        case 'gay':		
					...
					rate = body.slice(1)
					const ti =['4','9','17','28','34','48','59','2','62','74','83','97','100','29','94','75','82','41','39']
                ...
                case 'leveling':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply(`Use ${prefix}leveling on para ativar e  ${prefix}leveling off para desativar`)
					if (args[0] === 'on') {
              ...
              case 'antilink':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
					if (isAntiLink) return reply('O sistema já está ativado')
						antilink.push(from)
						...
						reply('1 para ativar e 0 para desativar.')
                                      ...
                                      case 'animealeatorio': 
			   case 'animerandom': 
				lucasssdomina1 = await fetchJson(`https://luc4rio.herokuapp.com/api/anime/aleatorio`)
				reply('*🌹 Enviando Seu Anime 🌹*')
				lucasssdomina2 = await getBuffer(lucasssdomina1.Imagem)
				lucasssdomina3 = `➸ *Nome Do Personagem:*\n*${lucasssdomina1.Nome}*\n\n➸ *Link: ${lucasssdomina1.Link_Pesquisa}*\n\n➢ *_Lucasss Domina 🎭💱_*`
				client.sendMessage(from, lucasssdomina2, image, {quoted: mek, caption: `${lucasssdomina3}`})
				break 
			   case 'waifu': 
			   case 'waifus': 
				lucasssdomina1 = await fetchJson(`https://luc4rio.herokuapp.com/api/anime/waifus`)
				reply('*🌹 Enviando Sua Waifu 🌹*')
				lucasssdomina2 = await getBuffer(lucasssdomina1.Resultado)
				lucasssdomina3 = `➸ *Link: ${lucasssdomina1.Resultado}*\n\n➢ *_Lucasss Domina 🎭💱_*`
				client.sendMessage(from, lucasssdomina2, image, {quoted: mek, caption: `${lucasssdomina3}`})
				break
				case 'cgame':
			...
			case 'clearall':
			    ...
			    case 'waifu':
					...
					break
					case 'phlogo':
reply('Calma tou fazendo chefe 😏')
wolflogo = body.slice(8)
wolf = wolflogo.split("|")[0];
logo = wolflogo.split("|")[090];
bufer = await getBuffer(`https://api.zeks.xyz/api/phlogo?text1=${wolf}&text2=${logo}&apikey=apivnz`)
client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tá na mão krai😌'})
break
case 'instagram':
                case 'imstagram':
			  if (args.length < 1) return reply('*Qual O Nome Do Perfil?? 🎭💱*')
			  lucasssdomina = body.slice(11)
				lucasssdomina2 = await fetchJson(`https://luc4rio.herokuapp.com/api/instagram?nomedeusuario=${lucasssdomina}`)
				reply('*⏳ Buscando Perfil ⏳*')
				lucasssdomina3 = await getBuffer(lucasssdomina2.Foto_Perfil)
				lucasssdomina4 = `*➸ Nome Completo :*\n*${lucasssdomina2.Nome}*\n*➸ Nome Do Usuário :*\n*${lucasssdomina2.Nome_Usuario}*\n*➸ Biografia :*\n*${lucasssdomina2.Biografia}*\n*➸ Seguindo : ${lucasssdomina2.Seguindo}*\n*➸ Seguidores : ${lucasssdomina2.Seguidores}*\n*➸ Número De Postagens : ${lucasssdomina2.Postagens}*\n\n➢ *_Lucasss Domina 🎭💱_*`
				client.sendMessage(from, lucasssdomina3, image, {quoted: mek, caption: `${lucasssdomina4}`})
				break
					case 'logohacker': 
		   case 'loguhacker': 
		         if (args.length < 1) return reply('*Digite Com O Texto Que Deseja ✓*')
				reply('*_⏳ Preparando Imagem ⏳_*')
				lucasssdomina = body.slice(12)
				lucasssdomina1 = await fetchJson(`http://luc4riio.herokuapp.com/api/textoanonymous?text=${lucasssdomina}`)
				lucasssdomina2 = await getBuffer(lucasssdomina1.Resultado)
				client.sendMessage(from, lucasssdomina2, image, {quoted: mek, caption: '*_Criador:_*\n*_DAVI DOMINA VADIA_*'})
				await limitAdd(sender)
				break 
			case 'logonaruto': 
		    case 'logunaruto': 
				...
				reply('*_⏳ Preparando Imagem ⏳_*')
				lucasssdomina = body.slice(12)
				lucasssdomina1 = await fetchJson(`http://luc4riio.herokuapp.com/api/textonaruto?text=${lucasssdomina}`)
				lucasssdomina2 = await getBuffer(lucasssdomina1.Resultado)
				client.sendMessage(from, lucasssdomina2, image, {quoted: mek, caption: '*_Resultado Do Efeito Para :_*\n*_Davi Domina_*'})
				await limitAdd(sender)
				break
			    case 'waifu2':
					reply(mess.wait)
				...
				case 'wibu':
					...
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS DAVI BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @iWouky domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*ESPERA UM POUCO*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*AQUI ESTÁ SUA MÚSICA*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
                await limitAdd(sender)
                break
				case 'setnome':
                   if (!isGroup) return reply(mess.only.group)
...
if(!isGroup) return reply(mess.only.group)
d = []
teks = 'AS MAIS PUTAS DESSE GRUPO😂\n'
for(i = 0; i < 5; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
...
if(!isGroup) return reply(mess.only.group)
d = []
teks = '𝙊𝙎 𝙈𝘼𝙄𝙎 𝙂𝘼𝙔𝙎 𝘿𝙊 𝙂𝙍𝙐𝙋𝙊𝙎️‍🏳️‍🌈\n'
for(i = 0; i < 5; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `️‍🏳️‍🌈❧ @${groupMembers[r].jid.split('@')[0]}\n`
d.push(groupMembers[r].jid)
...
for(i = 0; i < 1; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `️‍👉 @${groupMembers[r].jid.split('@')[0]}\n Parabéns vc é um adm agr, em 3 minutos vc não mandar up não vai vai ser adm`
d.push(groupMembers[r].jid)
}
mentions(teks, d, true)
} catch (e) {
...
case 'meulink':
  ...
  client.updatePresence(from, Presence.composing) 
      options = {
          text: `🚀salva zap🚀 \n\n_Me Chamo: *@${sender.split("@s.whatsapp.net")[0]}\n\n

🌞　　　　🛸　　　 　🌎　°　　🌓　•　　.°•　　　🚀 ✯✯　　
　　　★　*　　　　　°　　　　🛰　°·　　
.　　　•　°★　•
▁▂▃▄▅▆▇▇▆▅▄▃🌲Quiser Trocar divulgação vai no meu pv : *wa.me/${sender.split("@s.whatsapp.net")[0]}*\n*Ou*\n*api.whatsapp.com/send?phone=${sender.split("@")[0]}*`,
          contextInfo: { mentionedJid: [sender] }
    }
    client.sendMessage(from, options, text, { quoted: mek } )
				break
...
if(!isGroup) return reply(mess.only.group)
d = []
teks = 'OS MAIS PRETOS DESSE GRUPO🖤\n'
for(i = 0; i < 5; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `️‍🖤👉@${groupMembers[r].jid.split('@')[0]}\n`
d.push(groupMembers[r].jid)
...
reply('Deu erro, tente novamente :/')
       ...
       break
                case 'música2':
					...
					teks = body.slice(8)
					anu = await fetchJson(`https://videfikri.com/api/ytsearch/?query=${teks}`)
					r = Math.floor(Math.random() * 1 + 0)
					videosres = anu.result.data
					leg = `*✅ Música encontrada ✅*\n*Título: ${videosres[r].judul}*\n*Link: https://www.youtube.com/watch?v=${videosres[r].id}*\n*Duração: ${videosres[r].duration}*\n*Canal: ${videosres[r].channel}*\n*Publicado há: ${videosres[r].published_at}*\n*Views: ${videosres[r].views}*\n\n*Obs: Caso, não seja oq esteja procurando, seja mais especifico, não tenho bola de cristal*`
					url = videosres[r].thumbnail
					buff = await getBuffer(url.split('?')[0])
					await client.sendMessage(from, buff, image, {quoted: mek, caption: leg})
					if(videosres[r].duration.split(':').length > 2) return reply('*Só baixo musica com menos de 10 minutos*')
					if(videosres[r].duration.split(':')[0] > 10) return reply('*Só baixo musica com menos de 10 minutos*')
					reply('*⬇️ Baixando a música ⬇️*')
					musicanu = await fetchJson(`https://api.zeks.xyz/api/ytmp3?url=https://www.youtube.com/watch?v=${videosres[r].id}&apikey=apivinz`)
					buff = await getBuffer(musicanu.result.url_audio)
          ...
          reply('Espere..')
				...
				me=client.user 
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*AQUI ESTÁ SUA FIGURA*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
			...
			case 'dog':
				...
				case 'walpaperanime':
					...
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Davi ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT DAVI ✨* 」\n*│+ wa.me/558282042647*╰──────*「 *IWOUKY* 」*────*\n\n*_SE QUER SER ADMIN DO BOT LEVI_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'hentai':
					...
					case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRiKgSJ3bzO0Ahp6esPLf6ro_iW-TRhUczu6g&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia🙂'})
					break
					case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRHIi37qZB0YiQpouKwNGye1_nINQiMXpWcQg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'boa tarde, tropa 😎'})
					break
					case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQCQ3bNFFw_l4nd_PGe42Xi7tW63iP2t29KA&usqp=CAU`)
				...
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR9x6lmT3ImMuzIRxFs6tTAabvtLlJafS91vQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'boa noite tropa dorme bem😎'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSa-M3ix0ckDPe3TpRSLJ86CfXEaCV36ZGw_A&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia ou foda se'})
					break
				case 'termux':
					meme = await kagApi.memes()
           ...
           case 'shitlist':
               ...
               break
				case 'rr':
                    ...
                    break
                    case 'dono2':                     
					data = await fetchJson(`https://pastebin.com/raw/qXLF0ZhB`, {method: 'get'})
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSejmce7tjZqFD_iksZAdJ_rlN3F7vcgMANkk0LDSZYJWp3ECYyzLxjSpc&s=10`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* DAVI\n*YOUTUBE:* \n*WPP:* wa.me/+5582982042647\n*INSTA:* \n\n\nEspero que tenham gostado do bot '})
					break
					case 'sairpfv':
                    if (!isPremium) return reply(mess.only.premium)
				    if (!isGroup) return reply(mess.only.group)
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Tchauzinho Até algum dia😞', text) // ur cods
					}, 0)
                     break
                     case 'buc':
        ...
        case 'promover':
				...
				case 'admins':
					...
					client.updatePresence(from, Presence.composing)
					random = resposta[Math.floor(Math.random() * (resposta.length))]
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'kraii esse comando não tem', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
