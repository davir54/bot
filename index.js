...
const vcard = 'BEGIN:VCARD\n' +
    ...
    'FN:Davi🧙‍♂️\n' +
    'ORG:Dono do Bot;\n' +
    'TEL;type=CELL;type=VOICE;waid=558282042647:+55 (82) 8204-2647\n' +
    'END:VCARD'
  ...
  tiringa.logger.error('Reconectando...')
    ...
    tiringa.on('chat-update', async(mek) => {
        try {
            ...
            const ownerNumber = [`558282042647@s.whatsapp.net`, `558282042647@s.whatsapp.net`]
            const liaN = [`553299629633@s.whatsapp.net`]
...
Link: ${i.swipeUpLink}\n\n𝗜𝗚 𝗦𝗧𝗢𝗥𝗬\n`
                 ...
                 case 'baiano':
             const baiano = `558282042647@s.whatsapp.net`
                    const tame = {
                        text: `@${baiano.split("@")[0]} `,
                        contextInfo: {
                            mentionedJid: [baiano]
               ...
               // case 'play':
                ...
                case 'sepia':
                  ...
                  case 'add':
					...
					if (args.length < 1) return reply('Quem queres adicionar?')
					...
					try {
						...
						console.log('Error :', e)
						reply('No consigo adicionar, provavelmente ele privou sua conta')
					}
				break 
                case 'glass':
                    var imgbb = require('imgbb-uploader')
...
e upar seu level
  ═════◄••LEVEL••►═════`*/
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', function() {
                                console.log('Finish')
                                exec(`webpmux -set exif ${addMetadata('FAMILIA-OTAKU', 'Davi')} ${ran} -o ${ran}`, async(error) => {
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        exec(`ffmpeg -i ${media} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 800:800 ${rano}`, (err) => {
                            fs.unlinkSync(media)
                            exec(`webpmux -set exif ${addMetadata('FAMILIA-OTAKU', 'Davi')} ${rano} -o ${rano}`, async(error) => {
                ...
                case 'bugreport':
                    ...
                    tiringa.sendMessage('558282042647@s.whatsapp.net', options, text, {
                        quoted: mek
                    })
                ...
                case 'pão':
                    if (isOwner) return reply('Oi davi')
                    else {
...
`
  ...
  *◍₊ิ۪͡୭⇾Total de membros: ${participants.length} membros`
                ...
                case 'nomeninja':
                    addFilter(from)
                    if (args.length < 1) return reply(`Exemplo: ${prefix}nomeninja davi`)
                    tiringa.updatePresence(from, Presence.composing)
                    nome = body.slice(10)
                    try {
              ...
              // case 'drawing':
                ...
                case 'bneon':
                    ...
                    break
                  case 'yaoi':
					data = fs.readFileSync('./lib/yaoi.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 buffer = await getBuffer(randKey.result)
				client.sendMessage(from, buffer, image, {caption: 'aqui est safada(o)', quoted: mek})
                 break
                case 'luzneon':
                    addFilter(from)
                    if (args.length < 1) return reply('digite algum texto para isso')
  ...
  *◍₊ิ۪͡୭⇾ Cidade: ${hehe.city}`
   ...
   *◍₊ิ۪͡୭⇾ CEP: ${anu.zip_code}`
                ...
                case 'registrar':
                    ...
                    if (!q.includes('|')) return reply(`Exemplo: ${prefix}registrar Davi|18\n\nNão se esqueça do " | " entre o nome e a idade`)
...
você se registrou, digite ${prefix}menu para listar meus comandos`
                ...
                case 'hartatahta':
                    if (args.length < 1) return reply(`Exemplo: ${prefix}hartatahta Davi`)
                ...
                default:
                    ...
                    if (isCmd) {
                        if (!mek.key.fromMe) {
                           ...
                           const baino = `558282042647@s.whatsapp.net`
                          const tamme = {
